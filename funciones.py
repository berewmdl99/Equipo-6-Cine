...
async def eliminar_funcion(
    ...
    """Elimina una función si no tiene boletos vendidos."""
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail="Solo los administradores pueden eliminar funciones"
        )
    ...
    boletos = db.query(Boleto).filter(Boleto.funcion_id == funcion_id).all()
    if boletos:
        # Obtener información detallada de la función
        cantidad_boletos = len(boletos)
        mensaje = (
            f"No se puede eliminar la función '{funcion.pelicula.titulo}' en "
            f"la sala {funcion.sala.nombre} del {funcion.fecha.strftime('%d/%m/%Y')} "
            f"a las {funcion.hora.strftime('%H:%M')} porque tiene {cantidad_boletos} "
            f"boleto{'s' if cantidad_boletos != 1 else ''} vendido{'s' if cantidad_boletos != 1 else ''}"
        )
        raise HTTPException(
            status_code=400,
            detail=mensaje
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error al eliminar la función: {str(e)}"
        )
