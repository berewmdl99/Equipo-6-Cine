...
const ReportesVentas = () => {
  ...
  // Agrupar ventas por función
  const ventasAgrupadas = ventasFiltradas.reduce((acc, venta) => {
    const funcionKey = `${venta.funcion.id}`;
    if (!acc[funcionKey]) {
      acc[funcionKey] = {
      ...
      acc[funcionKey].cantidad += 1;
      acc[funcionKey].total += venta.precio;
    }
    return acc;
  }, {});
  ...
  const totalBoletos = ventasFiltradas.length;
  const totalIngresos = ventasFiltradas.reduce((total, venta) => total + venta.precio, 0);
  ...
  const descargarPDF = () => {
    ...
    try {
      ...
      doc.text(`Funciones con Ventas: ${ventasFormateadas.length}`, 14, 50);
      
      // Tabla de datos
      const tableColumn = ["Película", "Sala", "Fecha", "Boletos", "Precio", "Total"];
      const tableRows = ventasFormateadas.map(venta => [
        venta.pelicula,
        venta.sala,
  ...
  const descargarExcel = () => {
    ...
    try {
      ...
      const resumenData = [
        ...
        ["Funciones con Ventas", ventasFormateadas.length],
        [""],
        ["Filtros aplicados"],
        ...
        <ButtonGroup variant="outlined" aria-label="exportar reporte">
          <Tooltip title="Descargar PDF">
            <Button 
              ...
              disabled={loading || ventasFormateadas.length === 0}
            >
              PDF
            </Button>
          </Tooltip>
        ...
        {loading ? (
          ...
          <TableContainer>
            <Table stickyHeader>
              ...
              <TableBody>
                {ventasFormateadas.map((venta, index) => (
                  <TableRow key={index} hover>
                    <TableCell>{venta.pelicula}</TableCell>
                    <TableCell>{venta.sala}</TableCell>
